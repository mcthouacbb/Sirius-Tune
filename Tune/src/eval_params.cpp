...
void printParams(const EvalParams& params, std::ostream& os)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            ...
            for (int x = 0; x < 8; x++)
            {
                for (int k = 0; k < 4 - numDigits(data.psqtMG[i][y * 8 + x]); k++)
                {
                    os << ' ';
                }
                os << data.psqtMG[i][y * 8 + x] << ", ";
            }
            os << '\n';
        }
        os << "\t\t},\n";
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            ...
            for (int x = 0; x < 8; x++)
            {
                for (int k = 0; k < 4 - numDigits(data.psqtEG[i][y * 8 + x]); k++)
                {
                    os << ' ';
                }
                os << data.psqtEG[i][y * 8 + x] << ", ";
            }
            os << '\n';
        }
        os << "\t\t},\n";
...
int evaluate(const Board& board, const EvalParams& params, const EvalCache& cache)
{
    ...
    for (int sq = 0; sq < 64; sq++)
    {
        ...
        if (piece != PIECE_NONE)
        {
            ...
            bool isWhite = color == static_cast<int>(Color::WHITE);
            ...
            psqtMG[color] += data.psqtMG[type - 1][sq ^ (isWhite ? FLIP_Y : 0)];
            psqtEG[color] += data.psqtEG[type - 1][sq ^ (isWhite ? FLIP_Y : 0)];
        }
    }
    ...
    return (evalMG * (cache.totalPhase - phase) + evalEG * phase) / cache.totalPhase;
}